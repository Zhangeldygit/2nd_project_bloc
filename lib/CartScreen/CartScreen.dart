...
class _CartScreenState extends State<CartScreen> {
  ...
  void _setValues(List<Product> state) {
    sum = state.map((e) => e.count * e.price).reduce((value, element) => value + element);

    totalCount = state.map((e) => e.count).reduce((value, element) => value + element);
  }
                  ...
                  child: Column(
                    children: [
                      Padding(
                        ...
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [Text('Общая сумма $sum', style: TextStyle(fontWeight: FontWeight.bold),), Text('$totalCount вещи')],
                        ),
                      ),
                      Container(
                        width: 375,
